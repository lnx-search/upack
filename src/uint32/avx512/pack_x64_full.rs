...
pub(crate) unsafe fn to_u1(out: *mut u8, block: [__m512i; 4]) {
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u2(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u3(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u4(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u5(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u6(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u7(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u8(out: *mut u8, block: [__m512i; 4]) {
    let partially_packed = pack_u32_to_u8_unordered(block);
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u9(out: *mut u8, block: [__m512i; 4]) {
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u10(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u11(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u12(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u13(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u14(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u15(out: *mut u8, block: [__m512i; 4]) {
    let (hi, lo) = pack_u32_to_u16_split_unordered(block);
    unsafe { _mm512_storeu_epi8(out.add(0).cast(), lo) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u16(out: *mut u8, block: [__m512i; 4]) {
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u17(out: *mut u8, block: [__m512i; 4]) {
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u18(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u19(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u20(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u21(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u22(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u23(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u24(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u25(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u26(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u27(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u28(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u29(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u30(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u31(out: *mut u8, block: [__m512i; 4]) {
    unsafe { store_lo_u16_registers(out.add(0), block) };
...
/// - The runtime CPU must support the `avx512f` and `avx512bw` instructions.
pub(crate) unsafe fn to_u32(out: *mut u8, block: [__m512i; 4]) {
