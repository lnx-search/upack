...
/// - `pack_n` must be no greater than 64.
pub unsafe fn to_nbits(nbits: usize, out: *mut u8, block: [__m256i; 8], pack_n: usize) {
    debug_assert!(nbits <= 32, "BUG: invalid nbits provided: {nbits}");
    debug_assert!(nbits <= 32, "BUG: invalid nbits provided: {nbits}");
    const LUT: [unsafe fn(out: *mut u8, [__m256i; 8], usize); 33] = [
...
/// # Safety
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn to_u0(_out: *mut u8, _block: [__m256i; 8], _pack_n: usize) {}
