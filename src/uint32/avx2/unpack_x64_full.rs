...
pub unsafe fn from_u1(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u2(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u3(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u4(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u5(input: *const u8) -> [__m256i; 8] {
...
/// by `input`.
unsafe fn unpack_u5_registers(input: *const u8) -> [__m256i; 2] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u6(input: *const u8) -> [__m256i; 8] {
...
/// by `input`.
unsafe fn unpack_u6_registers(input: *const u8) -> [__m256i; 2] {
    let lo_bits = unsafe { unpack_u4_registers(input.add(0)) };
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u7(input: *const u8) -> [__m256i; 8] {
...
/// by `input`.
unsafe fn unpack_u7_registers(input: *const u8) -> [__m256i; 2] {
    let lo_bits = unsafe { unpack_u4_registers(input.add(0)) };
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u8(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u9(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u10(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u11(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u12(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u13(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u14(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u15(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x2(input.add(0)) };
    let lo_bits = unpack_u8_to_u16_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u16(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u17(input: *const u8) -> [__m256i; 8] {
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u18(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u19(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u20(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u21(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u22(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u23(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u24(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);

    let hi_bits = unsafe { load_si256x2(input.add(128)) };
    let mut hi_bits = unpack_u8_to_u32_unordered(hi_bits);
    hi_bits = slli_epi32::<16, 8>(hi_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u25(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);

    let hi_8bits = unsafe { load_si256x2(input.add(128)) };
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u26(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u27(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u28(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u29(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u30(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u31(input: *const u8) -> [__m256i; 8] {
    let lo_bits = unsafe { load_si256x4(input.add(0)) };
    let lo_bits = unpack_u16_to_u32_unordered(lo_bits);
...
///   bytes from.
/// - The runtime CPU must support the `avx2` instructions.
pub unsafe fn from_u32(input: *const u8) -> [__m256i; 8] {
...
mod tests {
    ...
    #[cfg(feature = "avx512")]
    #[rstest::rstest]
    #[case(1, avx512::pack_x64_full::to_u1, from_u1)]
    #[case(2, avx512::pack_x64_full::to_u2, from_u2)]
    #[case(3, avx512::pack_x64_full::to_u3, from_u3)]
    #[case(4, avx512::pack_x64_full::to_u4, from_u4)]
    #[case(5, avx512::pack_x64_full::to_u5, from_u5)]
    #[case(6, avx512::pack_x64_full::to_u6, from_u6)]
    #[case(7, avx512::pack_x64_full::to_u7, from_u7)]
    #[case(8, avx512::pack_x64_full::to_u8, from_u8)]
    #[case(9, avx512::pack_x64_full::to_u9, from_u9)]
    #[case(10, avx512::pack_x64_full::to_u10, from_u10)]
    #[case(11, avx512::pack_x64_full::to_u11, from_u11)]
    #[case(12, avx512::pack_x64_full::to_u12, from_u12)]
    #[case(13, avx512::pack_x64_full::to_u13, from_u13)]
    #[case(14, avx512::pack_x64_full::to_u14, from_u14)]
    #[case(15, avx512::pack_x64_full::to_u15, from_u15)]
    #[case(16, avx512::pack_x64_full::to_u16, from_u16)]
    #[case(17, avx512::pack_x64_full::to_u17, from_u17)]
    #[case(18, avx512::pack_x64_full::to_u18, from_u18)]
    #[case(19, avx512::pack_x64_full::to_u19, from_u19)]
    #[case(20, avx512::pack_x64_full::to_u20, from_u20)]
    #[case(21, avx512::pack_x64_full::to_u21, from_u21)]
    #[case(22, avx512::pack_x64_full::to_u22, from_u22)]
    #[case(23, avx512::pack_x64_full::to_u23, from_u23)]
    #[case(24, avx512::pack_x64_full::to_u24, from_u24)]
    #[case(25, avx512::pack_x64_full::to_u25, from_u25)]
    #[case(26, avx512::pack_x64_full::to_u26, from_u26)]
    #[case(27, avx512::pack_x64_full::to_u27, from_u27)]
    #[case(28, avx512::pack_x64_full::to_u28, from_u28)]
    #[case(29, avx512::pack_x64_full::to_u29, from_u29)]
    #[case(30, avx512::pack_x64_full::to_u30, from_u30)]
    #[case(31, avx512::pack_x64_full::to_u31, from_u31)]
    #[case(32, avx512::pack_x64_full::to_u32, from_u32)]
    #[cfg_attr(
