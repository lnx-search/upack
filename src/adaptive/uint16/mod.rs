use crate::adaptive::AdaptiveCompressibleArray;
...
pub const X128_MAX_OUTPUT_LEN: usize = <[u16; X128] as AdaptiveCompressibleArray>::MAX_OUTPUT_SIZE;
const DELTA_OVERHEAD: usize = size_of::<u16>();
...
pub const fn compressed_size(bit_length: usize, num_elements: usize) -> usize {
    crate::uint16::compressed_size(bit_length, num_elements) + DELTA_OVERHEAD
}

impl AdaptiveCompressibleArray for [u16; X128] {
    type CompressedBuffer = [u8; <Self as AdaptiveCompressibleArray>::MAX_OUTPUT_SIZE];
    type InitialValue = u16;
    const MAX_OUTPUT_SIZE: usize = (X128 * size_of::<u16>()) + DELTA_OVERHEAD;
...
) -> &mut [u8; crate::uint16::X128_MAX_OUTPUT_LEN] {
    const { assert!(X128_MAX_OUTPUT_LEN == crate::uint16::X128_MAX_OUTPUT_LEN + DELTA_OVERHEAD) };
